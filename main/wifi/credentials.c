...
wifi_credentials_t get_wifi_credentials(void)
{
  ...
  if (json_size == 0)
  {
    ESP_LOGW(LWIFICREDS, "config.json está vacío → usando defaults");
    return creds;
  }

  char *buffer = malloc(json_size + 1);
  if (!buffer)
  {
    ESP_LOGE(LWIFICREDS, "No hay memoria para copiar config.json");
    return creds;
  }
  ...
  if (!root)
  {
    ...
    ESP_LOGW(LWIFICREDS, "Error parseando config.json: %s", err ? err : "desconocido");
    return creds;
  }
